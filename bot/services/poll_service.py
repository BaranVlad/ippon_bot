...
from bot.services.training_config import get_next_training_date, load_trainings, WEEKDAY_SHORT
...
async def create_training_poll(bot: Bot, training: dict, training_date: date) -> None:
    ...
    question = f"Тренировка {weekday} {date_str} {training['time']}, {training['location']}"
    
    # Check if poll already exists
    existing = sheets.get_poll_by_date(date_str)
    if existing:
        logger.info(f"Poll for {date_str} already exists (poll_id={existing.get('poll_id')})")
        return
...
async def remind_non_voters(bot: Bot, training_date: str) -> None:
    ...
    poll = sheets.get_poll_by_date(training_date)
    if not poll:
...
async def send_poll_reminders(bot: Bot, poll: dict) -> None:
    ...
    votes = sheets.get_votes_by_poll(poll_id)
