...
TRAININGS_PATH = Path(__file__).parent.parent.parent / "data" / "trainings.json"

WEEKDAY_SHORT = ["пн", "вт", "ср", "чт", "пт", "сб", "вс"]


def load_trainings() -> List[Dict[str, Any]]:
    """Load training schedule from data/trainings.json."""
    ...
    # Filter enabled only by default
    enabled = [t for t in data if t.get("enabled", True)]
    logger.info(f"Loaded {len(enabled)}/{len(data)} enabled training schedules")
    return enabled
