...
| Конфиг | `resources/config/config.json` | Часы, дни, диапазоны | В репозитории |
...
`resources/config/config.json` — не содержит секретов, коммитится в репозиторий. При необходимости отредактируйте.
...
# Конфигурация
cp .env.example .env
# При необходимости отредактируйте resources/config/config.json (не содержит секретов)
...
**`resources/config/config.json`** (коммитится, не содержит секретов):
```json
{
...
│   ├── members.json.example   # Чувствительные данные (копируются в SENSITIVE_DIR)
│   ├── payment.json.example   # Чувствительные данные (копируются в SENSITIVE_DIR)
...
│   ├── middlewares/
│   │   └── membership.py    # Фильтр участников
│   ├── services/
│   │   ├── debt_notifier.py # Отправка напоминаний о долгах
│   │   └── poll_manager.py  # Создание опросов, напоминания
│   ├── data_loaders/
│   │   ├── members.py       # Загрузка members.json
│   │   ├── trainings.py     # Чтение trainings.json
│   │   ├── templates.py     # Загрузка шаблонов
│   │   └── payment.py       # Загрузка payment.json
│   ├── integrations/
│   │   └── gsheets/         # Работа с Google Таблицами
│   └── schedulers/
│       ├── debt_scheduler.py      # APScheduler (долги)
